...
struct BMP {
    ...
    void read(const char *fname) {
        std::string path = std::string("resources/") + fname;
        std::ifstream inp{ path, std::ios_base::binary };
        if (inp) {
            inp.read((char*)&file_header, sizeof(file_header));
            if (file_header.file_type != 0x4D42) { // BMP-Dateien beginnen mit "BM"
                throw std::runtime_error("Error! Unrecognized file format.");
            }

            inp.read((char*)&bmp_info_header, sizeof(bmp_info_header));

            // Der BMPColorHeader wird nur für transparente Bilder verwendet
            if (bmp_info_header.bit_count == 32) {
                if (bmp_info_header.size >= (sizeof(BMPInfoHeader) + sizeof(BMPColorHeader))) {
                    inp.read((char*)&bmp_color_header, sizeof(bmp_color_header));
                    check_color_header(bmp_color_header);
                } else {
                    std::cerr << "Error! The file \"" << path << "\" does not seem to contain bit mask information\n";
                    throw std::runtime_error("Error! Unrecognized file format.");
                }
            }
            ...
            int height = std::abs(bmp_info_header.height); // Höhe immer positiv
            int width = bmp_info_header.width;
            int bytes_per_pixel = bmp_info_header.bit_count / 8;
            ...
            if (bmp_info_header.width % 4 == 0) {
                if (bmp_info_header.height > 0) {
                    // Normale Reihenfolge (oben nach unten)
                    inp.read((char*)data.data(), data.size());
                } else {
                    // Vertikal gespiegelte Reihenfolge (unten nach oben)
                    for (int y = 0; y < height; ++y) {
                        inp.read((char*)(data.data() + row_stride * (height - 1 - y)), row_stride);
                    }
                }
            } else {
